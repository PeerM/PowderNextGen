...
object Cell {
  ...
  def apply(particles: Iterable[Powder], lastState: IndexedSeq[Cell], linearPos: Int, lineSize: Int, totalSize: Int): Cell = {
    ...
    val maxOption = Util.linearizedNeighbours(linearPos, lineSize, math.min(lastState.size, totalSize))
      .map(pair => (pair._1, lastState(pair._2).pressureAbs))
